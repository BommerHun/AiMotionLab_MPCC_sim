...
args_forward = {}
args_forward["vehicle_params"] = params["parameter_server"]["ros__parameters"]["vehicle_params"]
args_forward["MPCC_params"] = params["parameter_server"]["ros__parameters"]["controllers"]["MPCC"]
args_forward["drive_bridge"] = params["parameter_server"]["ros__parameters"]["drive_bridge"]
args_forward["crazy_observer"] = params["parameter_server"]["ros__parameters"]["crazy_observer"]
MOTOR_LIMIT = args_forward["drive_bridge"]["MOTOR_LIMIT"]
...
with open(file_name) as file:
    params = yaml.full_load(file)
args_reverse = {}
args_reverse["vehicle_params"] = params["parameter_server"]["ros__parameters"]["vehicle_params"]
args_reverse["MPCC_params"] = params["parameter_server"]["ros__parameters"]["controllers"]["MPCC"]
args_reverse["drive_bridge"] = params["parameter_server"]["ros__parameters"]["drive_bridge"]
args_reverse["crazy_observer"] = params["parameter_server"]["ros__parameters"]["crazy_observer"]
...
t_end = car0_trajectory_forward.evol_tck[0][-1]

        
t_eval=np.linspace(0, t_end, 100)
s=splev(t_eval, car0_trajectory_forward.evol_tck)

(x,y) = splev(s, car0_trajectory_forward.pos_tck)
...
car0_controller_reverse = mpcc_reverse_controller(vehicle_params= args_reverse["vehicle_params"], MPCC_params= args_reverse["MPCC_params"], index = 69)
...
s = np.linspace(0, car0_controller_forward.trajectory.L,10000)



plotter.set_ref_traj(np.array(car0_controller_forward.trajectory.spl_sx(s)), np.array(car0_controller_forward.trajectory.spl_sy(s)))
...
while( not (simulator.glfw_window_should_close()) & (car0_controller_forward.finished == False)): # the loop runs until the window is closed
    ...
    (x0, args_forward) = car0_controller_forward.simulate(states = st, inputs = car0_controller_forward.input, dt = car0_controller_forward.MPCC_params["Tf"]/ car0_controller_forward.MPCC_params["N"])
    ...
    if car0_trajectory_forward.is_finished() or car0_controller_forward.finished == True:
        break
...
while( not (simulator.glfw_window_should_close()) & (car0_controller_reverse.finished == False)): # the loop runs until the window is closed
    if GUI:
        simulator.update()
    else:
        simulator.update_()
...
s = np.linspace(0, car0_controller_forward.trajectory.L,1000)

plt.figure()
plt.plot(car0_controller_forward.trajectory.spl_sx(s), car0_controller_forward.trajectory.spl_sy(s))
plt.axis('equal')
plt.xlabel("x[m]")
plt.ylabel("y[m]")
plt.grid(True)
